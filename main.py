...
from models import *
from utils.utils import showTimeInfo
...
parser.add_argument('--model', default='stackhourglass',
                    help='select model')
parser.add_argument('--datapath', default="./dataset/SceneFlow",  # '/media/jiaren/ImageNet/SceneFlowData/',
                    help='datapath')
...
def main():
    ...
    for epoch in range(0, args.epochs):
        ...
        for batch_idx, (imgL_crop, imgR_crop, disp_crop_L) in enumerate(TrainImgLoader):
            ...
            showTimeInfo(100, args.epochs, epoch, len(TrainImgLoader), batch_idx,
                         time.time() - start_full_time, time.time() - start_time, loss)

        print('Epoch %d total training loss = %.3f' % (
        epoch, total_train_loss / len(TrainImgLoader)))  # len(TrainImgLoader)表示Batch的个数。
