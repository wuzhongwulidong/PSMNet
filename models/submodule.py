...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.firstconv = nn.Sequential(convbn(3, 32, 3, 2, 1, 1),   # H/2
                                       nn.ReLU(inplace=True),
        ...
        self.layer1 = self._make_layer(BasicBlock, 32, 3, 1, 1, 1)  # conv1_x: (ResBock, 通道数, 个数, stride, pad, dilation):
        self.layer2 = self._make_layer(BasicBlock, 64, 16, 2, 1, 1)  # conv2_x   H/4
        self.layer3 = self._make_layer(BasicBlock, 128, 3, 1, 1, 1)  # conv3_x
        self.layer4 = self._make_layer(BasicBlock, 128, 3, 1, 1, 2)  # conv4_x   H/4

        self.branch1 = nn.Sequential(nn.AvgPool2d((64, 64), stride=(64, 64)),
                                     convbn(128, 32, 1, 1, 0, 1),  # 1x1卷积 + Bn
                                     nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        output_raw = self.layer2(output)  # conv2_16: [B, C=64, H/4, w/4]
        output = self.layer3(output_raw)
        output_skip = self.layer4(output)  # conv4_3: [B, C=128, H/4, w/4]
        ...
        output_feature = torch.cat(
            (output_raw, output_skip, output_branch4, output_branch3, output_branch2, output_branch1), 1)  # H/4
